...
document.addEventListener('DOMContentLoaded', function() {
                ...
                if (response.ok) {
                    ...
                    if (strategyData) {
                        ...
                        strategyDisplaySection.style.display = 'block'; // Show the strategy display section
                         window.toast.success('Strategy generated successfully!', 'success');
    ...
    if (downloadStrategyBtn) {
        downloadStrategyBtn.addEventListener('click', async function() {
            ...
            if (!selectedGoalId) {
                window.toast.error('error', 'Please select a goal first to generate a PDF.');
                return;
            }
            if (!riskAppetite) {
                window.toast.error('error', 'Risk appetite is not set. Please generate a strategy first.');
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    window.toast.success('Download successfully', 'success');
                } else {
                    const errorData = await response.json();
                    console.error('Error downloading PDF:', errorData);
                    window.toast.error('error', `Failed to download PDF: ${errorData.message || 'Unknown error'}`);
                }
            } catch (error) {
                console.error('Network error during PDF download:', error);
                window.toast.error('error', 'Network error or unexpected issue during PDF download.');
            }
        });
    }
        ...
        implementStrategyBtn.addEventListener('click', async function() {
            ...
            try{
                ...
                if(data.status === 'success'){
                    ...
                    window.toast.success('Investment strategy saved successfully!','success');
                }else{
